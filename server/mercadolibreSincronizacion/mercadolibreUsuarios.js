...
class MercadolibreUsuarios extends  MercadolibreGeneric {
    ...
    get(argObjId){
        return new Promise(function(respData,respRej){
            try {
                this.verificaCredenciales( argObjId ) ;
                //
                let tempUserId = (typeof argObjId=="object" ) ? argObjId.id : parseInt(argObjId) ;
                let mlReq      = {method:'GET',uri: '',contentType: 'application/json',accept: 'application/json' } ;
                mlReq.uri = 'https://api.mercadolibre.com/users/'+tempUserId+( this.tokenId ? '?access_token='+this.tokenId : '' ) ;
                console.log('uri: '+mlReq.uri) ;
                //
